const Task = require('../models/Task');

// @desc  Get all available (Open) tasks
// @route GET /api/talent/tasks/available
// @access Talent
const getAvailableTasks = async (req, res) => {
  try {
    // Show tasks that are either unassigned OR specifically assigned to this user
    const tasks = await Task.find({
      status: 'Open',
      $or: [{ assignedTo: null }, { assignedTo: req.user._id }]
    })
      .populate('createdBy', 'name')
      .sort({ createdAt: -1 });

    res.json(tasks);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// @desc  Get tasks assigned to the logged-in talent
// @route GET /api/talent/tasks/mine
// @access Talent
const getMyTasks = async (req, res) => {
  try {
    // Only show claimed/submitted tasks here. Open tasks remain in Available Tasks.
    const tasks = await Task.find({
      assignedTo: req.user._id,
      status: { $ne: 'Open' }
    })
      .sort({ updatedAt: -1 });

    res.json(tasks);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// @desc  Claim an open task
// @route PUT /api/talent/tasks/:id/claim
// @access Talent
const claimTask = async (req, res) => {
  try {
    const task = await Task.findOneAndUpdate(
      {
        _id: req.params.id,
        status: 'Open',
        $or: [{ assignedTo: null }, { assignedTo: req.user._id }]
      },
      { status: 'Claimed', assignedTo: req.user._id },
      { new: true }
    );

    if (!task) {
      // If task wasn't updated, check if it even exists to give a proper 404 vs 409
      const existingTask = await Task.findById(req.params.id);
      if (!existingTask) {
        return res.status(404).json({ message: 'Task not found' });
      }
      return res.status(409).json({ message: 'Task has already been claimed.' });
    }

    res.json(task);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

module.exports = { getAvailableTasks, getMyTasks, claimTask };
