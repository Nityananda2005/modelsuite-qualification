...
const SubmissionReviewModal = ({ submission, onClose, onReviewed }) => {
  ...
  // Support both new and old submissions
  const files =
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="bg-bg-surface rounded-lg px-4 py-3.5 border border-border">
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              {task.status && (
                <span
                  className={`inline-block px-2 py-[2px] rounded-full text-[11px] font-medium status-badge-${task.status}`}
                >
                  {task.status}
                </span>
              )}
              ...
              <div className="flex flex-col gap-2">
                {files.map((url, index) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2.5 bg-bg-surface border border-border rounded-lg px-3 py-2 hover:bg-bg-hover transition"
                  >
          ...
          <div className="flex gap-3 pt-1 border-t border-border mt-1">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 bg-bg-input text-text-muted border border-border rounded-lg text-sm font-medium hover:bg-bg-hover hover:text-text-primary transition-all"
            >
              Cancel
            </button>
